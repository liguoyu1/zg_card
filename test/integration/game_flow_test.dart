...
void main() {
  const testHero = Hero(
    id: 'H_TEST', name: '测试英雄', className: 'test', kingdom: 'test', heroPowerName: '铁壁', heroPowerDescription: '获得2护甲',
    skillType: SkillType.defensive,
  );
  const opponentHero = Hero(
    id: 'H_OPP', name: '敌将', className: 'bingjia', kingdom: '齐', heroPowerName: '铁壁', heroPowerDescription: '获得2护甲',
    skillType: SkillType.defensive,
  );
  ...
  group('2 圣盾交互', () {
    GameState makeGs(Card atk, Card def) => GameState(
      ...
      player2: Player(id:'p2', hero:opponentHero, board:[def], mana:10),
      activePlayerId: 'p1',
    );

    test('2.1 普通vs普通', () {
      final c = GameRules.resolveCombat(
        const Card(id:'a',name:'a',type:CardType.minion,cost:2,attack:3,health:3,description:'',owner:CardOwner.neutral,rarity:Rarity.common),
        const Card(id:'b',name:'b',type:CardType.minion,cost:2,attack:2,health:4,description:'',owner:CardOwner.neutral,rarity:Rarity.common),
      );
    ...
    test('2.2 圣盾vs普通', () {
      const atk = Card(id:'a',name:'a',type:CardType.minion,cost:2,attack:3,health:3,
        keywords:[Keyword.divineShield],description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      const def = Card(id:'b',name:'b',type:CardType.minion,cost:2,attack:2,health:4,
    ...
    test('2.5 剧毒vs圣盾', () {
      const atk = Card(id:'a',name:'a',type:CardType.minion,cost:2,attack:1,health:1,
        keywords:[Keyword.poisonous],description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      const def = Card(id:'b',name:'b',type:CardType.minion,cost:2,attack:1,health:4,
        keywords:[Keyword.divineShield],description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      final r = bf.minionAttack(makeGs(atk, def), 'p1', atk, def.id);
  ...
  // ═══════════════════════════════════════════════════════
  group('4 AI行为', () {
    const minion = Card(id:'C1',name:'步',type:CardType.minion,cost:3,attack:3,health:3,
      description:'',owner:CardOwner.neutral,rarity:Rarity.common);
    const spell = Card(id:'C2',name:'火',type:CardType.spell,cost:4,
      description:'',owner:CardOwner.neutral,rarity:Rarity.common);

    test('4.1 Normal随从优先', () {
      final ai = AIController();
      final order = ai.getOptimalPlayOrder([spell, minion], const Player(id:'p',hero:testHero,mana:10));
      expect(order.first.isMinion, isTrue);
    });

    test('4.2 嘲讽优先攻击', () {
      final ai = AIController();
      const taunt = Card(id:'T',name:'T',type:CardType.minion,cost:2,attack:1,health:6,
        keywords:[Keyword.taunt],description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      const self = Player(id:'p1',hero:testHero,board:[minion]);
      const opp = Player(id:'p2',hero:opponentHero,board:[taunt, minion]);
  ...
  group('5 边界条件', () {
    test('5.1 满手牌爆牌', () {
      final full = List.generate(10, (i) => Card(id:'C$i',name:'$i',type:CardType.minion,
        cost:1,attack:1,health:1,description:'',owner:CardOwner.neutral,rarity:Rarity.common));
      var gs = GameState(
        player1:Player(id:'p1',hero:testHero,hand:full,deck:[full.first],mana:5,maxMana:5),
        player2:const Player(id:'p2',hero:opponentHero),
        activePlayerId:'p1');
    ...
    test('5.2 满场拒绝', () {
      final full = List.generate(7, (i) => Card(id:'B$i',name:'$i',type:CardType.minion,
        cost:1,attack:1,health:1,description:'',owner:CardOwner.neutral,rarity:Rarity.common));
      const newCard = Card(id:'new',name:'新',type:CardType.minion,cost:1,attack:1,health:1,
        description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      expect(GameRules.canPlayCard(newCard, Player(id:'p1',hero:testHero,board:full,mana:10)), isFalse);
    });

    test('5.3 0法力拒绝', () {
      const c = Card(id:'c',name:'c',type:CardType.minion,cost:5,attack:5,health:5,
        description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      expect(GameRules.canPlayCard(c, const Player(id:'p1',hero:testHero,hand:[c])), isFalse);
    });
    ...
    test('5.5 平局', () {
      final r = GameRules.checkGameEnd(
        const Player(id:'p1',hero:testHero,health:0),
        const Player(id:'p2',hero:opponentHero,health:0));
      expect(r, isNull);
    });
  ...
  group('6 组合技能', () {
    test('6.1 合纵连横', () {
      const a = Card(id:'Z008',name:'苏秦',type:CardType.minion,cost:4,attack:3,health:3,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.zonghengjia,rarity:Rarity.rare);
      const b = Card(id:'Z009',name:'张仪',type:CardType.minion,cost:4,attack:3,health:3,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.zonghengjia,rarity:Rarity.rare);
      const w = Card(id:'W',name:'W',type:CardType.minion,cost:2,attack:2,health:2,
        description:'',owner:CardOwner.neutral,rarity:Rarity.common);
      const gs = GameState(
        player1:Player(id:'p1',hero:testHero,hand:[a,b],board:[w],mana:10),
        player2:Player(id:'p2',hero:opponentHero),
        activePlayerId:'p1');
    ...
    test('6.2 机关术', () {
      const a = Card(id:'M008',name:'墨子',type:CardType.minion,cost:7,attack:5,health:5,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.mojia,rarity:Rarity.legendary);
      const b = Card(id:'M009',name:'公输班',type:CardType.minion,cost:5,attack:4,health:4,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.mojia,rarity:Rarity.epic);
      const gs = GameState(
        player1:Player(id:'p1',hero:testHero,hand:[a,b],mana:10),
        player2:Player(id:'p2',hero:opponentHero),
        activePlayerId:'p1');
      final combos = ComboSystem.getActivatedCombos(gs.player1.hand);
      final r = ComboSystem.executeCombo(combos.first, gs, 'p1');
    ...
    test('6.3 儒道双修', () {
      final deck = List.generate(10, (i) => Card(id:'D$i',name:'$i',type:CardType.minion,
        cost:1,attack:1,health:1,description:'',owner:CardOwner.neutral,rarity:Rarity.common));
      const a = Card(id:'R008',name:'孔子',type:CardType.minion,cost:8,attack:6,health:6,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.rujia,rarity:Rarity.legendary);
      const b = Card(id:'D008',name:'老子',type:CardType.minion,cost:7,attack:5,health:5,
        description:'',keywords:[Keyword.battlecry],owner:CardOwner.daojia,rarity:Rarity.legendary);
      final gs = GameState(
        player1:Player(id:'p1',hero:testHero,hand:const [a,b],deck:deck,mana:10,health:20),
        player2:const Player(id:'p2',hero:opponentHero),
        activePlayerId:'p1');
      final combos = ComboSystem.getActivatedCombos(gs.player1.hand);
      final r = ComboSystem.executeCombo(combos.first, gs, 'p1');
