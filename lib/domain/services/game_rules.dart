...
class GameRules {
  ...
  static ({int attackerDamage, int defenderDamage}) resolveCombat(
    ...
    final int defenderDamage = attacker.attack;
...
class BattlefieldService {
  ...
  GameState playCard(GameState state, String playerId, Card card, {String? targetId}) {
    ...
    // 从手牌移除（按 id 匹配，不依赖 Equatable 对比）
    final newHand = List<Card>.from(player.hand)..removeWhere((c) => c.id == card.id);
  ...
  GameState heroAttack(GameState state, String playerId) {
    ...
    // 每回合只能砍一次
    if (weapon.hasAttackedThisTurn) return state;

    // 装甲先吸收伤害
    var newArmor = opponent.armor;
    var newHealth = opponent.health;
    if (newArmor >= weapon.attack) {
      newArmor -= weapon.attack;
    } else {
      newHealth -= (weapon.attack - newArmor);
      newArmor = 0;
    }

    // 消耗1点耐久
    final newDurability = weapon.health - 1;
    final Card? newWeapon = newDurability > 0
  ...
  GameState minionAttackHero(GameState state, String playerId, Card attacker) {
    ...
    if (newArmor >= dmg) {
      ...
      newHealth -= (dmg - newArmor);
      newArmor = 0;
    }

    final newBoard = List<Card>.from(currentPlayer.board);
    ...
    if (attacker.hasLifesteal && dmg > 0) {
      ...
      newState = newState.updatePlayer(updatedPlayer.copyWith(health: updatedPlayer.health + dmg));
    }
