...
import { verifyApplePurchase } from './apple';
...
// 赠送策略（与 shop_screen.dart _gemBonus 一致）
function iapGemBonus(base: number): number {
  const map: Record<number, number> = { 60: 0, 300: 50, 600: 150, 1500: 500, 3000: 1500 };
  return map[base] ?? 0;
}

/** Apple IAP 收据验证 + 加钻 + 幂等 (通过 Apple transactionId) */
export async function verifyIapApple(
  playerId: string,
  receipt: string,
  productId: string,
  ...
  // 1. Apple 校验
  const appleResult = await verifyApplePurchase(receipt, productId, transactionId);
  if ('error' in appleResult) return { error: appleResult.error };
  const txId = appleResult.transactionId;
  ...
  if (existing) {
    const player = await prisma.player.findUnique({ where: { id: playerId } });
    if (!player) return { error: 'player not found' };
    return { success: true, alreadyProcessed: true, gems: player.gems, gold: player.gold, balanceVersion: player.balanceVersion };
  }

  // 3. 映射钻石数
  const base = GEM_BASE_MAP[productId];
  if (!base) return { error: `Unknown product: ${productId}` };
  const total = base + iapGemBonus(base);

  // 4. 乐观锁写入
  const player = await prisma.player.findUnique({ where: { id: playerId } });
  if (!player) return { error: 'player not found' };
  const newBalance = player.gems + total;

  await prisma.$transaction([
    prisma.player.update({
      where: { id: playerId, balanceVersion: player.balanceVersion },
      data: { gems: newBalance, balanceVersion: { increment: 1 } },
    }),
    prisma.transaction.create({
      data: {
        playerId, type: 'earn_gems', currency: 'gem', amount: total,
        balanceAfter: newBalance, detail: `Apple IAP ${productId}`, externalId: txId,
      },
    }),
  ]);

  return { success: true, gems: newBalance, balanceVersion: player.balanceVersion + 1, gold: player.gold };
}
